...
void solve() {
    ...
    // Probability that row will be lucky for given s
    ll rowProb = 1;

    for(ll s = 1; s <= min(n, k); ++s){
        rowProb = add(mult(rowProb, sub(1, p)), mult(sub(1, rowProb), p));
        ll es = mult(com(n, s), power(rowProb, m));
        ans = add(ans, es);
    }
