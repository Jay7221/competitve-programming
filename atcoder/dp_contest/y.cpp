...
void solve() {
    ...
    modint res;
    res = com(h - 1 + w - 1, h - 1);
    vector<modint> contrib;
    for(auto [x, y]: points){
        contrib.push_back(modint(com(x + y, x)));
    }

    for(int i = 0; i < n; ++i){
        int dx = h - 1 - points[i].x;
        int dy = w - 1 - points[i].y;
        res -= contrib[i] * com(dx + dy, dx);
        for(int j = i + 1; j < n; ++j){
            if((points[j].x >= points[i].x) && (points[j].y >= points[i].y)){
                int dx = points[j].x - points[i].x;
                int dy = points[j].y - points[i].y;
                contrib[j] -= contrib[i] * com(dx + dy, dx);
            }
        }
    }
